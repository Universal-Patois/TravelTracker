...
describe("Traveler", () => {
  ...
  it("should greet a user by their first name", () => {
    expect(traveler1.greetUser()).to.equal(
      `Welcome back Emmet! The World Awaits You!`
    );
    expect(traveler2.greetUser()).to.equal(
      `Welcome back Emeline! The World Awaits You!`
    );
  });

  it("should get the users past trips", () => {
    traveler1.getPastTrips(
      sampleTripsRepo.trips,
      sampleDestinations.allDestinations
    );
    expect(traveler1.pastTrips).to.deep.equal([
      {
        id: 1,
  ...
  it("should get the users annual spending", () => {
    expect(
      traveler1.getAnnualSpending(
        sampleTripsRepo.trips,
        sampleDestinations.allDestinations
      )
    ).to.equal(`You Have Spent $24469.5 On Travel So Far This Year!`);
  });
});
